...
public class AppRepositoryImpl implements AppRepository {
    ...
    public User getUser(String email) {
        return userList.stream().filter(user -> user.getEmail().equals(email)).findFirst().orElseThrow(UserNotFoundException::new);
    }
    ...
    public Activity getActivity(String email, String activityName) {
        return getUser(email).getActivities().stream().filter(act -> act.getActivityName().equals(activityName)).findFirst().orElseThrow(ActivityIsNotFoundException::new);
    }
    ...
    public Admin getAdmin(String email) {
        return adminlist.stream().filter(admin -> admin.getEmail().equals(email)).findFirst().orElseThrow(AdminIsNotFoundException::new);
    }
