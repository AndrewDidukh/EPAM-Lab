...
public interface UserApi {
    ...
    ActivityDto getActivity(@PathVariable String email, @PathVariable String activity);

    @ApiImplicitParam(name = "email", paramType = "path", required = true, value = "User email")
    @ApiOperation("Update Activity")
    @ResponseStatus(HttpStatus.OK)
    @PatchMapping(value = "/{email}/updateActivity")
    ActivityDto updateActivity(@PathVariable String email, @RequestBody @Validated(OnUpdate.class) ActivityDto activityDto);
    ...
    ActivityDto addActivityTime(@PathVariable String email, @PathVariable String activity, @PathVariable Long seconds);
